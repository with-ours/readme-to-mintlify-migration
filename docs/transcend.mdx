---
title: "Transcend Implementation Guide"
description: "Learn how to integrate Transcend with Ours Privacy for seamless consent management and compliance with privacy regulations worldwide."
---

# Transcend Implementation Guide

Transcend and Ours Privacy work together to provide a comprehensive consent management solution that supports healthcare, GDPR, CCPA, and other privacy regulations. This guide explains the capabilities, operating modes, and integration patterns available when using Transcend as your Consent Management Platform (CMP).

## Overview

Transcend controls when the Ours Privacy SDK loads and provides granular consent data that flows through your entire data pipeline. This approach delivers:
...
* **Flexible Integration**: Works with any Transcend configuration or consent strategy

## Operating Modes

Ours Privacy supports three distinct operating modes for consent management with Transcend, each offering different levels of data collection and consent enforcement:

### Mode 1: Strict Consent Mode (Zero Data Collection)

Ours Privacy SDK is **not loaded** until explicit consent is granted through Transcend, ensuring zero data collection before user permission.

**How it works:**

* Transcend controls when the Ours Privacy SDK loads
...
Use Transcend's web SDK to get the consent status or grab their cookie string if it contains the allowed/denied categories. Then pass this information into event properties and visitor properties:

<CodeGroup>
  ```bash bash
  // Get consent status from Transcend
  const consentStatus = { 
    marketing: true, 
    analytics: false, 
    functional: false, 
    necessary: true 
  }; // Example object grab the real one with the Transcend web sdk.

  // Track consent change event
  ours('track', 'consent_change', {
    // Event properties
    ...consentStatus,
    cmp_vendor: 'transcend'
  }, {
...
Ours Privacy's mapping layer routes events based on Transcend consent:
...
Integrating Transcend with Ours Privacy enables:
...
For additional support or questions about Transcend integration, contact our support team at [\[email protected\]](/cdn-cgi/l/email-protection#8dfef8fdfde2fff9cde2f8fffefdffe4fbeceef4a3eee2e0).
...
* * [Transcend Implementation Guide](#transcend-implementation-guide)
